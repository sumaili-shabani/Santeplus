<div class="table-responsive resultat_ok">
     <div align="right">
        <!-- <button type="button" id="add_button" data-toggle="modal" data-target="#userModal" class="btn btn-round btn-default btn-lg"><i class="fa fa-plus"></i>Ajouter</button>  -->
      </div>
    <br>
  <table class="table-striped  nk-tb-list nk-tb-ulist dataTable no-footer" data-auto-responsive="false" id="user_data" role="grid" aria-describedby="DataTables_Table_1_info">  
   <thead>
      <tr>
        <th width="10%">
            <div class="form-group" >
                <a href="javascript:void(0)" class="btn btn-success btn-sm btn-circle select_all" id="#select_all" data-toggle="modal" data-target="#userModal3"><i class="fa fa-send"></i></a>
            </div>
        </th>
        <th width="15%">Nom</th>
        <th width="20%">Sujet</th>
        <th width="20%">Email</th>
        <th width="20%">message</th>
        <th width="10%">mise à jour</th>
        <th width="5%">fichier</th>
        <th width="5%">voir+</th>
        <th width="5%">del</th>
      </tr>
    </thead>
    <tbody id="example-tbody">
      
    </tbody>
    <tfoot>
      <tr>
        <th width="10%">
            <div class="form-group" >
                <a href="javascript:void(0)" class="btn btn-success btn-sm btn-circle select_all" id="#select_all" data-toggle="modal" data-target="#userModal3"><i class="fa fa-send"></i></a>
            </div>
        </th>
        <th width="15%">Nom</th>
        <th width="20%">Sujet</th>
        <th width="20%">Email</th>
        <th width="20%">message</th>
        <th width="10%">mise à jour</th>
        <th width="5%">fichier</th>
        <th width="5%">voir+</th>
        <th width="5%">del</th>
      </tr>
    </tfoot>
 </table>
</div>